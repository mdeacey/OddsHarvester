...
async def test_scrape_historic(url_builder_mock, setup_scraper_mocks):
    ...
    scraper._collect_match_links = AsyncMock(return_value=["https://oddsportal.com/match1", "https://oddsportal.com/match2"])
...
async def test_scrape_upcoming(url_builder_mock, setup_scraper_mocks):
    ...
    # Mock the URLBuilder
    url_builder_mock.get_upcoming_matches_url.return_value = "https://oddsportal.com/football/england/premier-league/matches/20230601"
    
    # Mock methods
    scraper._prepare_page_for_scraping = AsyncMock()
    scraper.extract_match_links = AsyncMock(return_value=["https://oddsportal.com/match1", "https://oddsportal.com/match2"])
    scraper.extract_match_odds = AsyncMock(return_value=[{"match": "data1"}, {"match": "data2"}])
...
async def test_collect_match_links(setup_scraper_mocks):
    ...
    assert sorted(result) == sorted(["https://oddsportal.com/match1", "https://oddsportal.com/match2", "https://oddsportal.com/match3"])
