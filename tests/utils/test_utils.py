...
from src.utils.sport_market_constants import (
    ...
    BaseballMarket, BaseballOverUnderMarket
)
...
@pytest.mark.parametrize("sport_str_mixed_case, expected", [
    ("FooTbAlL", EXPECTED_MARKETS[Sport.FOOTBALL]),
    ("TENNIS", EXPECTED_MARKETS[Sport.TENNIS]),
    ("BaseBall", EXPECTED_MARKETS[Sport.BASEBALL]),
])
...
@pytest.mark.parametrize("invalid_sport, expected_error", [
    ("invalid_sport", "Invalid sport name: invalid_sport. Expected one of "),
    ("handball", "Invalid sport name: handball. Expected one of "),
    (123, "Unsupported sport: 123"),
    (None, "Unsupported sport: None"),
    ([], "Unsupported sport: []"),
    ({}, "Unsupported sport: {}"),
])
def test_get_supported_markets_invalid_sport(invalid_sport, expected_error):
    """Test handling of invalid sports and input types."""
